...
const neighbours = {
  encode: function (obj) {
    return [
      obj.peers.map((peer) => {
        return endpoint.encode(peer.endpoint).concat(peer.id)
      }),
      timestamp.encode(obj.timestamp)
    ]
  },
...
const packet = {
  ...
  decode: function (buffer) {
    ...
    let sighash = SHA3(typedata)
    let publicKey = secp256k1.recover(sighash, signature, buffer[96], false).slice(1)
